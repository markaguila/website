...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    rolls, sum, rolled_one = 0, 0, False
...
def free_bacon(score):
    ...
    if score < 10:
        ...
        dig_one, dig_two = score // 10, score % 10
        return abs(dig_one - dig_two) + 2
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            ...
            result = take_turn(dice_to_roll, score1, dice)
            score0 += result
        else:
            dice_to_roll = strategy1(score1, score0)
            result = take_turn(dice_to_roll, score0, dice)
            score1 += result
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        one_pt_msg = "point! That's the biggest gain yet for Player"
        more_pt_msg = "points! That's the biggest gain yet for Player"
        if gain <= previous_high:
            return announce_highest(who, previous_high, score)
        else:
            if gain > 1:
                print(gain, more_pt_msg, who)
            else:
                print(gain, one_pt_msg, who)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for rolls in range(1, 11):
        avg = make_averaged(roll_dice, num_samples)(rolls, dice)
        if avg > largest_sum:
            largest_sum = avg
            largest_roll = rolls
    return largest_roll
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    bacon_pts = free_bacon(opponent_score)
    bacon_to_swap = is_swap(score + bacon_pts, opponent_score)
    should_bacon = bacon_to_swap and (score < opponent_score)
    if should_bacon or (bacon_pts >= margin):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
